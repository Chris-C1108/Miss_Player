    ...
    monkey({
      ...
      userscript: {
        ...
        description: {
          ...
          ja: 'MissAV広告ブロック|片手モード|MissAV自動詳細表示|MissAV自動高画質|MissAVリダイレクト対応|MissAV自動ログイン|カスタムプレーヤー|jable po*nhubなどに対応した多言語サポート',
          vi: 'MissAV không quảng cáo|chế độ một tay|MissAV tự động mở rộng chi tiết|MissAV tự động chất lượng cao|Hỗ trợ chuyển hướng MissAV|MissAV tự动 đăng nhập|trình phát tùy chỉnh|hỗ trợ đa ngôn ngữ cho jable po*nhub v.v.'
        },
        author: 'Chris_C',
        match: [
          '*://*.missav.ws/*',
  ...
  build: {
    ...
    rollupOptions: {
      output: {
        format: 'iife',
        globals: {
          'virtual:monkey-css': 'virtual_monkey_css'
        }
      }
    }
  }

...
`;
	...
	var VideoStateSwitcher = class {
		constructor(options = {}) {
			...
			this.indicatorWidth = Math.min(128, Math.max(100, Math.round(this.config.width * .5)));
			this.slotWidth = Math.min(108, Math.max(86, Math.round(this.config.width * .4)));
		...
		snapToIndex(index, animated = true, triggerSound = true, animConfig = null) {
			...
			const defaultDuration = Math.round(.32 / Math.sqrt(stiffnessRatio) * 100) / 100;
			const defaultEasing = safeStiffness > 380 ? "cubic-bezier(0.14, 1.05, 0.24, 1)" : "cubic-bezier(0.2, 0.98, 0.25, 1)";
		...
		bindEvents() {
			...
			const forceEndDrag = (clientX) => {
				...
				if (wasDragging) {
					...
					const momentumFactor = Math.round(160 * (.55 / safeFriction));
					...
					const threshold = Math.round(this.slotWidth * Math.max(.15, .28 - (stiffnessRatio - 1) * .08));
					let step = 0;
					...
					const baseDuration = .32 / Math.sqrt(stiffnessRatio);
					const animDuration = Math.min(.55, Math.max(.18, baseDuration + speed * (.08 / safeFriction)));

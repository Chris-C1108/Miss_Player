...
`;
	...
	var WebDavClient = class {
		...
		static request(options) {
			const { method = "GET", url, user = "", pass = "", headers = {}, data = null, timeout = 12e3 } = options;
		...
		static async testConnection(config) {
			...
			try {
				...
				if (res.status === 401 || res.status === 403) throw new Error(`认证失败 (${res.status}): 请检查用户名与密码/Token`);
				if (res.status >= 500) throw new Error(`服务器错误 (${res.status})`);
				if (res.status === 200 || res.status === 404 || res.status === 204 || res.status === 207) return {
					success: true,
					message: "WebDAV 连接成功！"
				};
				return {

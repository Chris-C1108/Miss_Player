...
`;
	...
	var CommentPanel = class CommentPanel {
		...
		initDelegatedEvents() {
			...
			this.uiElements.playerContainer.addEventListener("touchmove", (e) => {
				...
				const touch = e.touches[0];
				if (Math.hypot(touch.clientX - touchStartX, touch.clientY - touchStartY) > 8) clearLongPress();
			}, { passive: true });
		...
		handleTimeClick(secs) {
			...
			if (this.targetVideo) {
				...
				if (lm && typeof lm.setDraftTabFromTime === "function") lm.setDraftTabFromTime(secs);
		...
		renderCommentCard(c, isNew = false) {
			...
			const countdownBtn = hasTimestamps ? `<button class="jc-countdown-btn${isCountdownActive ? " jc-countdown-btn--active" : ""}" title="${isCountdownActive ? "当前已按总时长换算为正向时间，点击还原" : "按视频总时长倒数换算为正向时间"}">${__("commentCountdown") || "时间倒数"}</button>` : "";
	...
	var LoopManager = class {
		...
		init(elements) {
			...
			if (this.tabScrollContainer) this.tabScrollContainer.addEventListener("wheel", (e) => {
				if (e.deltaY !== 0) {
					e.preventDefault();

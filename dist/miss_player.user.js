...
`;
	...
	var SCRIPT_VERSION = typeof GM_info !== "undefined" && GM_info?.script?.version ? GM_info.script.version : "5.6.30";
	...
	var CommentPanel = class CommentPanel {
		...
		initDelegatedEvents() {
			...
			this.uiElements.playerContainer.addEventListener("touchstart", (e) => {
				...
				if (!link) {
					this.uiElements.playerContainer.querySelectorAll(".jc-time-link.show-add-badge").forEach((el) => el.classList.remove("show-add-badge"));
					return;
				}
		...
		handleAddTabFromTime(secs) {
			const lm = this.getLoopManager();
			if (!lm) {
				Toast("循环控制面板未就绪", 2e3, "error");
				return;
			}
			if (typeof lm.setDraftTabFromTime === "function") {
				lm.setDraftTabFromTime(secs);
				Toast(Array.isArray(secs) && secs.length >= 2 ? `已将区间 ${formatSeconds(secs[0])} ~ ${formatSeconds(secs[1])} 填入控制栏草稿胶囊` : `已将时间 ${formatSeconds(Array.isArray(secs) ? secs[0] : secs)} 填入控制栏草稿胶囊`, 2e3, "info");
				if (this.uiManager) this.uiManager.showControls();
			}
		}

...
`;
		...
		_showCommentReportModal(comment) {
			...
			submitBtn.addEventListener("click", async () => {
				...
				if (webdavConfig && webdavConfig.url) try {
					...
					Toast("上报成功并已同步至 WebDAV！", 2e3, "success");
				} catch (e) {
	...
	var VolumeController = class {
		...
		createVolumeSlider(container) {
			...
			if (this.supportsVolumeControl) volumeControl.appendChild(sliderContainer);
			else volumeControl.classList.add("tm-volume-control-no-slider");
			...
			volumeButton.addEventListener("click", (e) => {
				...
				if (this.supportsVolumeControl && !isExpanded) {
					...
					if (this.targetVideo.volume === 0 || this.targetVideo.muted) {
						this.targetVideo.muted = false;
						if (this.supportsVolumeControl) this.targetVideo.volume = this.lastVolume;
					} else if (this.supportsVolumeControl) {
						this.lastVolume = this.targetVideo.volume;
						this.targetVideo.volume = 0;
					} else this.targetVideo.muted = true;
					this.updateVolumeUI();
				}
			});
		...
		updateVolumeUI() {
			...
			if (volumeButton) {
				...
				if (!playerState || playerState.settings?.betaSafariMuteStyle !== false) volumeButton.classList.toggle("is-muted", isMuted);
				else volumeButton.classList.remove("is-muted");
			}
			if (volumeButton) {
			...
			if (this.volumeValue) {
				...
				if (volume === 0 || this.targetVideo.muted) this.volumeValue.classList.add("volume-muted");
				else if (volume < .3) this.volumeValue.classList.add("volume-low");
				else if (volume < .7) this.volumeValue.classList.add("volume-medium");
				else this.volumeValue.classList.add("volume-high");
			}
	...
	var VideoStateSwitcher = class {
        ...
        `;
            ...
            box-shadow: inset 0 1px 2px rgba(0, 0, 0, 0.25), 0 2px 10px rgba(0, 0, 0, 0.25);
        ...
        `;
            ...
            box-shadow: inset 0 1px 0.5px rgba(255, 255, 255, 0.45), inset 0 -0.5px 0.5px rgba(255, 255, 255, 0.1), 0 4px 16px rgba(0, 0, 0, 0.5);
        ...
        `;
            ...
            `;
                ...
                transition: opacity 0.2s cubic-bezier(0.16, 1, 0.3, 1), color 0.2s ease, text-shadow 0.2s ease;
                pointer-events: none;
		...
		updateVariantUI() {
			this.itemElements.forEach((item, idx) => {
				...
				item.labelEl.style.textShadow = isActive ? "0 1px 2px rgba(0, 0, 0, 0.6)" : "none";
                ...
                `;
					...
					if (curMode === "normal") {
						...
						btn.title = "点击取消单片段循环";
						btn.innerHTML = `<span style="font-size:12px; font-weight:600; color:#ff453a; white-space:nowrap; letter-spacing:-0.2px; text-shadow:0 1px 2px rgba(0,0,0,0.5);">取消循环</span>`;
					} else if (this.runtimeState.isTourEnded) {
						btn.title = "从第一个胶囊重新播放";
						btn.innerHTML = `
                            <span style="font-size:12px; font-weight:600; color:#ffffff; display:flex; align-items:center; gap:4px; white-space:nowrap; text-shadow:0 1px 2px rgba(0,0,0,0.5);">
                                ${REPLAY || ""} 重新播放
                            </span>
                        `;
						const svg = btn.querySelector("svg");
						...
						const statusDot = this.runtimeState.isPlaying ? "<span style=\"display:inline-block; width:4px; height:4px; border-radius:50%; background:#30d158; box-shadow:0 0 6px rgba(48,209,88,0.8); margin-left:3px;\"></span>" : "<span style=\"font-size:10px; margin-left:2px; opacity:0.8;\">▶</span>";
						btn.title = modeText + " (点击" + (this.runtimeState.isPlaying ? "暂停" : "继续") + ")";
						btn.innerHTML = `<span style="font-size:12px; font-weight:600; color:#ffffff; display:flex; align-items:center; white-space:nowrap; letter-spacing:-0.1px; text-shadow:0 1px 2px rgba(0,0,0,0.6);">${modeText}${statusDot}</span>`;
					}
					btn.addEventListener("click", (e) => {
						e.stopPropagation();
		...
		snapToIndex(index, animated = true, triggerSound = true, animConfig = null) {
			...
			const defaultDuration = Math.round(.3 / Math.sqrt(stiffnessRatio) * 100) / 100;
		...
		bindEvents() {
			...
			picker.addEventListener("mouseenter", () => {
				this.indicator.style.transform = "translate(-50%, 0) scale(1.025)";
				this.indicator.style.boxShadow = "inset 0 1px 0.5px rgba(255, 255, 255, 0.55), inset 0 -0.5px 0.5px rgba(255, 255, 255, 0.15), 0 6px 20px rgba(0, 0, 0, 0.6)";
				this.capsuleContainer.style.borderColor = "rgba(255, 255, 255, 0.15)";
			});
			picker.addEventListener("mouseleave", () => {
				this.indicator.style.transform = "translate(-50%, 0) scale(1)";
				this.indicator.style.boxShadow = "inset 0 1px 0.5px rgba(255, 255, 255, 0.45), inset 0 -0.5px 0.5px rgba(255, 255, 255, 0.1), 0 4px 16px rgba(0, 0, 0, 0.5)";
				this.capsuleContainer.style.borderColor = "rgba(255, 255, 255, 0.08)";
			});
			const forceEndDrag = (clientX) => {
				...
				if (wasDragging) {
					...
					const baseDuration = .3 / Math.sqrt(stiffnessRatio);
					const animDuration = Math.min(.5, Math.max(.18, baseDuration + speed * (.07 / safeFriction)));
		...
		setProgressFill(pct) {
			...
			if (this.modes[this.currentIndex]?.id === "normal" || pct <= 0) {
				this.indicator.style.background = "linear-gradient(180deg, rgba(255, 255, 255, 0.22) 0%, rgba(255, 255, 255, 0.10) 100%)";
				return;
			}
			this.indicator.style.background = `linear-gradient(to right, rgba(255, 120, 130, 0.40) ${pct}%, rgba(255, 255, 255, 0.16) ${pct}%)`;
		}
	};
	var PlaybackController = class {
		...
		createPlayPauseButton(container) {
			...
			this.stateSwitcher = new VideoStateSwitcher({
				...
				onModeChange: (newMode) => {
					...
					} else if (prevMode === "normal") this._isTourEnded = false;
		...
		handleMainPlayButtonClick() {
			...
			if (isPaused) {
				if (!this._isCapsuleLoopPlaying) this.startCapsulePlayback(this._capsulePlayIndex || 0);

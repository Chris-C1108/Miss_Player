...
`;
	...
	var PlaybackController = class {
		...
		toggleCapsulePlayback() {
			...
			} else this.startCapsulePlayback();
		}
		...
		_showModeToast() {
			Toast(`胶囊播放: ${this._capsulePlayMode === "preview" ? "走马灯预览 (各播30秒)" : "高潮回看 (完整区间)"}`, 3e3, "info", () => {
				Toast("【预览模式】所有胶囊统一只播30秒快进预览\n【回看模式】单点时间戳播60秒，时间区间完整播放A至B点", 4e3, "info");
			});
		}
		_playCurrentCapsule() {
			...
			if (this._capsulePlayMode === "preview") this._currentCapsuleDuration = 30;
